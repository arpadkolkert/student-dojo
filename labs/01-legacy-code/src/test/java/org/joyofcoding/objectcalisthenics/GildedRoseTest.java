...
import org.joyofcoding.objectcalisthenics.ItemClases.ConcertItem;
import org.joyofcoding.objectcalisthenics.ItemClases.HamerItem;
import org.joyofcoding.objectcalisthenics.ItemClases.ItemInt;
...
public class GildedRoseTest {
    ...
    public void after_one_day_with_sufuras_having_sellIn_lesser_than_zero_and_quality_greater_than_zero() {
        items = new ArrayList<ItemInt>();
        items.add(new HamerItem("Sulfuras, Hand of Ragnaros", -1, 1));
        repeatUpdateQuality(1);
    ...
    private List<ItemInt> aBunchOfBackstagePasses() {
        List<ItemInt> listOfPasses = new ArrayList<ItemInt>();
        for (int i = 0; i < 100; i++) {
    ...
    private ItemInt aRandomBackstagePass() {
        int quality = randomQuality();
        int sellIn = randomSellIn();
        return new ConcertItem("Backstage passes to a TAFKAL80ETC concert", sellIn, quality);
    }
}
